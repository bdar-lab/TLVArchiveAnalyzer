...
This script receives:
 1) Table of building documents info generated by TLVArchive project
 2) Table of buildings with the true-year of construction beginning

The script predict the true years of all other records, using the following model-
 1) It defines categories by grouping types of documents,
 2) It calculates weights for each category frequency from true-year table, using linear model.
 3) It predict true-years for all records.

It prints the output into csv.
"""
...
CATEGORY1 = ["‘ŸÍË-Í€‡ŸÍ ◊Í’ﬁ‘", "‘ŸÍË ﬁŸ‹’‹Ÿ ◊Í’›"]
CATEGORY2 = ["‘’”‚Í È’ﬁ‘ ‚‹ ‘Ÿÿ‹ ‘È—◊‘,–“Ë’Í —‡Ÿ‘,È’—ËŸ ÍÈ‹’›"]
CATEGORY3 = ["Í‚’”Í “ﬁË", "ÍŸÁ Í‚’”Í “ﬁË", "ÿ’‰· 4", "ÿ’‰· 1"]
...
# Sets min limit to avoid predicting true-years with too few documents
...
# Receives csvs with true years of beginning constructions and documents info
# Transform the data to a pivoted table, grouped by tik_id, where types are fields
# Export pivoted table into csv and parquet
def pivot_csvs(years_csv, document_csv, output_csv, output_parquet):
    # Read csv1 into a DataFrame
    log(f"Reading {years_csv}..")
    true_years_df = pd.read_csv(years_csv)

    # Read csv2 into a DataFrame
    log(f"Reading {document_csv}..")
    docs_df = pd.read_csv(document_csv)

    # Group csv2 by 'tik_id' and 'type', then aggregate 'date' into a list of uniqe years
    log(f"Grouping {document_csv}..")
    docs_df['year'] = pd.to_datetime(docs_df['date'], format='%d/%m/%Y', errors='coerce').dt.year
    docs_df_grouped = docs_df.groupby(['tik_id', 'type'])['year'].apply(list).reset_index()

    # Step 5: Pivot the grouped df2 to have one column per document 'type' with a list of dates
    log(f"Pivoting grouped {document_csv}..")
    docs_df_pivoted = docs_df_grouped.pivot_table(index='tik_id', columns='type', values='year', aggfunc=lambda x:x).reset_index()

    # Step 6: Merge the results from df1 (years) and df2 (document types) based on 'tik_id'
    # Merge df1 (with years) with df2_pivoted (with document dates per type)
    log(f"Merging grouped data..")
    df3 = pd.merge(docs_df_pivoted, true_years_df, on='tik_id', how='left')

    # First, drop duplicates in docs_df based on 'tik_id' and keep the first occurrence of each tik_id
    docs_df_unique = docs_df.drop_duplicates(subset='tik_id', keep='first')[DISPLAY_COLUMNS]
    df3 = pd.merge(df3, docs_df_unique, on='tik_id', how='left')

    # Step 7: Write the resulting dataframe to a new CSV (csv3)
    log(f"Writing {output_csv}..")
    df3.to_csv(output_csv, index=False, encoding='utf-8-sig')

    # Save to Parquet file
    log(f"Writing {output_parquet}..")
    df3.to_parquet(output_parquet, engine='pyarrow', index=False)
    return df3
